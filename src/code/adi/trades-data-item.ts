...
export class TradesDataItem extends MarketSubscriptionDataItem implements TradesDataItem.UsableBadnessRecordAccess {
    ...
    constructor(marketsService: MarketsService, myDataDefinition: DataDefinition) {
        const tradesSubscriptionDefinition = myDataDefinition as TradesDataDefinition;
        const market = marketsService.getDataMarketOrUnknown(tradesSubscriptionDefinition.marketZenithCode);
    ...
    private createRecord(change: TradesDataMessage.AddUpdateChange) {
        const marketZenithCode = change.marketZenithCode;
        let market: DataMarket | undefined;
        if (marketZenithCode !== undefined) {
            market = this._marketsService.getDataMarketOrUnknown(marketZenithCode);
        }
