...
import { BrokerageAccountRecordsSubscriptionDataDefinition } from './common/internal-api';
...
export abstract class AllBrokerageAccountRecordsDataItem<Record extends BrokerageAccountRecord>
    ...
    private addAccountWrappers(accountsIndex: Integer, addCount: Integer) {
        ...
        for (let i = accountsIndex; i < accountsNextRangeStartIdx; i++) {
            ...
            const recordsDefinition = this.createRecordsDataDefinition(account.zenithCode);
            const recordsDataItem = this.subscribeDataItem(recordsDefinition) as RecordsBrokerageAccountSubscriptionDataItem<Record>;
    ...
    private createOneOrMoreAccountsInErrorBadness(accountZenithCode: string) {
        const badness: Badness = {
            reasonId: Badness.ReasonId.OneOrMoreAccountsInError,
            reasonExtra: accountZenithCode,
        };
        return badness;
    }

    protected abstract createRecordsDataDefinition(accountZenithCode: string): BrokerageAccountRecordsSubscriptionDataDefinition;
}
...
export namespace AllBrokerageAccountRecordsDataItem {
    ...
    export class AccountWrapper<Record extends BrokerageAccountRecord> {
        ...
        get accountMapKey() { return this._dataItem.accountZenithCode; }
        get accountCode() { return this._dataItem.accountCode; }
