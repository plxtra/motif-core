...
export class BrokerageAccountBalancesDataItem extends RecordsBrokerageAccountSubscriptionDataItem<Balances> implements BrokerageAccountGroupRecordList<Balances> {
    ...
    private processDataMessageAddUpdateChange(
        ...
        const accountZenithCode = change.accountZenithCode;
        if (accountZenithCode !== this.accountZenithCode) {
            throw new ZenithDataError(ErrorCode.BABDIPDMAUC133330444, JSON.stringify(change));
        } else {
            let accountId: BrokerageAccountEnvironmentedId;
            const account = this.account;
            if (account !== undefined) {
                accountId = account.id;
            } else {
                accountId = BrokerageAccountEnvironmentedId.createFromZenithCode(this._marketsService, accountZenithCode);
            }

            const mapKey = Balances.generateMapKey(accountId, change.currencyId);
            if (last !== undefined && last.mapKey === mapKey) {
                ...
                const item = addUpdateDeleteItemMap.mergeAddUpdateChange(change, mapKey, isAdd);
                last = {
                    mapKey,
                    item,
                };
    ...
    private processDataMessageInitialiseAccountChange(
        ...
        if (accountZenithCode !== this.accountZenithCode) {
            ...
            for (let j = 0; j < this.count; j++) {
                ...
                const mapKey = Balances.generateMapKey(accountId, currencyId);
        ...
        for (let i = 0; i < changeCount; i++) {
            ...
            switch (change.typeId) {
                case BalancesDataMessage.ChangeTypeId.AddUpdate: {
                    ...
                    last = this.processDataMessageAddUpdateChange(addUpdateChange, addUpdateDeleteItemMap, last);
                    break;
                }
                case BalancesDataMessage.ChangeTypeId.InitialiseAccount: {
                    ...
                    last = this.processDataMessageInitialiseAccountChange(initialiseChange, addUpdateDeleteItemMap, last);
                    break;
                }
                default:
                    throw new UnreachableCaseError('BDICAUDIM69429', change.typeId);
            }
        }
    ...
    private createSortedAddUpdateDeleteItemList(map: BalancesDataItem.AddUpdateDeleteItemMap) {
    ...
    private processAddItems(itemList: BalancesDataItem.AddUpdateDeleteList) {
        ...
        if (account === undefined) {
            ...
            this.checkUsableNotifyListChange(UsableListChangeTypeId.Insert, addStartIdx, itemCount);
        }
    }
    ...
    private processUpdateItems(itemList: BalancesDataItem.AddUpdateDeleteList) {
        ...
        const items = itemList.items;
        for (let i = itemList.firstUpdateIndex; i < nextItemListSectionIdx; i++) {
            const item = items[i];
            const mapKey = item.mapKey;
    ...
    private processBalancesMessage(msg: BalancesDataMessage) {
        ...
        const addUpdateDeleteItemMap = this.createAddUpdateDeleteItemMap(changes);

        if (addUpdateDeleteItemMap.size > 0) {
            const list = this.createSortedAddUpdateDeleteItemList(addUpdateDeleteItemMap);
...
export namespace BalancesDataItem {
    ...
    export class AddUpdateDeleteItemMap extends Map<MapKey, BalancesDataItem.AddUpdateDeleteItem> {
        mergeAddUpdateChange(change: BalancesDataMessage.AddUpdateChange, mapKey: MapKey, isAdd: boolean) {
    ...
    export class AddUpdateDeleteList extends ComparableList<AddUpdateDeleteItem> {
